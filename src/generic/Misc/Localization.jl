...
@doc Markdown.doc"""
     Localization(R::AbstractAlgebra.Ring, primes::Vector{T}; cached=true) where {T <: RingElement}
...
"""
function Localization(R::AbstractAlgebra.Ring, primes::Vector{T}; cached=true) where {T <: RingElement}
   prime = R(prod(primes))
