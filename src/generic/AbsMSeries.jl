...
function base_ring(R::AbsMSeriesRing{T}) where T <: RingElement
    return base_ring(R.poly_ring)::parent_type(T)
end
...
"""
number_of_variables(R::AbsMSeriesRing) = number_of_variables(R.poly_ring)

number_of_generators(R::AbsMSeriesRing) = number_of_generators(R.poly_ring)
...
function (R::AbsMSeriesRing)()
    if R.weighted_prec == -1
        ...
        return R(R.poly_ring(), [0 for i in 1:nvars(R)])
    end
end
...
function (R::AbsMSeriesRing{T})(x::T) where T <: RingElem
    if R.weighted_prec == -1
        ...
        return R(R.poly_ring(x), [0 for i in 1:nvars(R)])
    end
end
...
function (R::AbsMSeriesRing)(b::Union{Integer, Rational, AbstractFloat})
    if R.weighted_prec == -1
        ...
        return R(R.poly_ring(b),  [0 for i in 1:nvars(R)])
    end
end
