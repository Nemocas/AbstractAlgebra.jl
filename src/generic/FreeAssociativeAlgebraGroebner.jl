...
@doc """
    groebner_basis(g::Vector{FreeAssociativeAlgebraElem{T}}, reduction_bound::Int = typemax(Int), remove_redundancies::Bool = false; gb::Vector{FreeAssociativeAlgebraElem{T}})
...
One can provide the keyword argument `gb` with a precomputed partial Groebner basis. 
It is assumed (but not checked) that all entries of `gb` are indeed contained in the ideal generated by `g`.
...
julia> g = AbstractAlgebra.groebner_basis([(a*b*a*b^2)^2 - 1]; gb = [f1,f2,f3])
