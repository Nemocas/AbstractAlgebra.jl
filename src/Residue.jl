...
type ResidueRing{T <: Union(RingElem, BigInt)} <: Ring
   base_ring::Ring
   modulus::T

   function ResidueRing(R::Ring, modulus::T)
      return try
         ModulusDict[modulus]
      catch
...
function Base.call(a::ResidueRing{BigInt})
   z = Residue{BigInt}(ZZ())
   z.parent = a
   return z
end

function Base.call(a::ResidueRing{BigInt}, b::Integer)
   z = Residue{BigInt}(mod(ZZ(b), modulus(a)))
   z.parent = a
   return z
end

Base.call(a::ResidueRing{BigInt}, b::Residue{BigInt}) = b
...
function ResidueRing(R::IntegerRing, el::Integer)
   ...
   return ResidueRing{BigInt}(R, R(el))
end