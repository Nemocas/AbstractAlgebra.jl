...
function deepcopy(a::fmpz_abs_series)
   z = fmpz_abs_series(a)
   z.prec = a.prec
   z.parent = parent(a)
   return z
...
function show(io::IO, x::fmpz_abs_series)
   if length(x) == 0
      print(io, "0")
   else
      cstr = ccall((:fmpz_poly_get_str_pretty, :libflint), Ptr{UInt8}, 
        (Ptr{fmpz_abs_series}, Ptr{UInt8}), &x, bytestring(string(var(parent(x)))))
...
Base.promote_rule{T <: Integer}(::Type{fmpz_abs_series}, ::Type{T}) = fmpz_abs_series

Base.promote_rule(::Type{fmpz_abs_series}, ::Type{fmpz}) = fmpz_abs_series
...
function Base.call(a::FmpzAbsSeriesRing)
   z = fmpz_abs_series()
   z.prec = a.prec_max
   z.parent = a
   return z
...
function Base.call(a::FmpzAbsSeriesRing, b::Integer)
   if b == 0
      z = fmpz_abs_series()
      z.prec = a.prec_max
   else
...
function Base.call(a::FmpzAbsSeriesRing, b::fmpz)
   if b == 0
      z = fmpz_abs_series()
      z.prec = a.prec_max
   else
...
function Base.call(a::FmpzAbsSeriesRing, b::fmpz_abs_series)
   parent(b) != a && error("Unable to coerce power series")
   return b
end

function Base.call(a::FmpzAbsSeriesRing, b::Array{fmpz, 1}, len::Int, prec::Int)
...
function PowerSeriesRing(R::FlintIntegerRing, prec::Int, s::AbstractString{}; model=:capped_relative)
   S = Symbol(s)

   if model == :capped_relative
      parent_obj = FmpzRelSeriesRing(prec, S)
   elseif model == :capped_absolute
      parent_obj = FmpzAbsSeriesRing(prec, S)
   end
