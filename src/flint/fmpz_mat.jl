...
function cansolve(a::fmpz_mat, b::fmpz_mat)
   ...
   b = deepcopy(b)
   z = MatrixSpace(ZZ, cols(b), cols(a))()
   l = min(rows(a), cols(a))
   for i=1:cols(b)
     for j=1:l
       k = 1
...
> Return true a matrix $x$ and a matrix $k$ uch that $ax = b$ and the columns
> of $k$ for a basis for the nullspac of $a$ or false and two matrices 
> otherwise.
"""
function cansolve_with_nullspace(a::fmpz_mat, b::fmpz_mat)
   ...
   N =  MatrixSpace(FlintZZ, cols(a), 0)(), 0
...
> Return a tuple $(x, d)$ consisting of a column vector $x$ such that
> $ax = db$ where $x$ and $b$ are column vectors with the same number
> of rows as the $a$. Note that $a$ must be a square matrix. If these
> conditions are not met, an exception is raised. Typically, $d$ will be
