...
function divexact(x::MatrixElem, y::Union{Integer, Rational, AbstractFloat}; check::Bool=false)
   z = similar(x)
   for i = 1:nrows(x)
      for j = 1:ncols(x)
...
function divexact(x::MatrixElem{T}, y::T; check::Bool=false) where {T <: RingElem}
   z = similar(x)
   for i = 1:nrows(x)
      for j = 1:ncols(x)
         z[i, j] = divexact(x[i, j], y; check=check)
