...
function from_univar(a, var::Int, Kxyz)
  r = MPolyBuildCtx(Kxyz)
  ev = zeros(Int, nvars(Kxyz))
  for i in degree(a):-1:0
    ev[var] = i
...
function pfracinit(
  ...
  betas = [E[zero(R) for i in 1:r] for j in 0:l]
  ...
  inv_prod_betas1 = E[zero(R) for i in 1:r]

  # working space that need not be reallocated on each recursive call
  deltas = [E[zero(R) for i in 1:r] for j in 0:l]
...
function hlift_without_lcc(
  ...
  # this is apparently also supposed to work with 1 factor
  if r < 2
...
function hliftstep_quartic(
  ...
  newfac = E[zero(R) for i in 1:r]

  Blen = zeros(Int, r)
  B = [E[] for i in 1:r]
  U = [E[zero(R) for j in 1:liftdegs[m] + 1] for i in 1:r]
  for i in 1:r
    taylor_get_coeffs!(B[i], fac[i], xalpha)
    Blen[i] = length(B[i])
...
# Return A/b^bexp
function divexact_pow(A::Fac{E}, b::E, bexp::Int; check::Bool=false) where E
