...
@deprecate map_coeffs(f, p::MPolyElem; cached = true, parent = Generic._change_mpoly_ring(parent(f(zero(base_ring(p)))), parent(p), cached)) map_coefficients(g, p; cached = cached, parent = parent)
