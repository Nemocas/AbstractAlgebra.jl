...
(Free associative algebra over Finite field F_2 on x, y, u, v, t, s, AbstractAlgebra.Generic.FreeAssAlgElem{AbstractAlgebra.GFElem{Int64}}[x, y, u, v, t, s])
...
0
 ...
 To use this for the normal form computation, one uses the exponent vectors of the generators of 
 the groebner basis as keywords.
 ```jldoctest; setup = :(using AbstractAlgebra.Generic, AbstractAlgebra)
 ...
 julia> aut = AhoCorasickAutomaton([g_i.exps[1] for g_i in g])
 julia> normal_form(u*(x*y)^3*s*t + u*(x*y)^2*s*t +u*s*t + v*s*t, g, aut)
 0
 ```
