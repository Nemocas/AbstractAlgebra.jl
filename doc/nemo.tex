...
\subsection{Generic polynomials ($T[x]$) : \code|Poly\{T\}|}
...
The Flint type for polynomials over $\Z/n\Z$ is \code{fmpz_poly}. Polynomials of this type are created
using the \code{PolynomialRing} constructor. They are automatically created if the modulus $n$ doesn't
fit into a \code{UInt}.
...
In the constructors below $S$ is assumed to be a \code{FmpzModPolyRing} as constructed above.
