...
division of fractions an integer (!) and \code{%} does not correspond in any meaningful way with \code{/}
and all sorts of other really messy things.

The division functions available in Julia are just depressing for an algebraist. They choose the convention
they do so that \code{(1/2) * 2 != 0}, which is the Python 3 convention. There is the \code{div} function
for Euclidean division. But it is defined for fractions!

They should have stuck with either the C convention for \code{/} and \code{%} or decided to go with floating
...
an impossible inverse is encountered, an error is thrown.}
...
In example 1 above the principal ideal is $(n)$ and in example 2 the principal
ideal is $(f)$

We don't require the ideal $(r)$ to be maximal (so that the residue ring
is a field) or prime (so that the residue ring is an integral domain). 

Instead, we allow $r$ to be any nonzero element of $R$, but raise an error every
time a computation in $R/(r)$ requires computing an impossible inverse.
