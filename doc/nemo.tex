...
could be a \code{Symbol} for example (there is currently no way to restrict it to be a \code{Symbol}). 

There is a slight problem when we are defining methods whose signatures don't depend on that
...
introspection, and undocumented runtime type deconstruction to get access to that symbol.
