...
could be a \code{Symbol} for example. 

One immediate problem is that in Julia there is currently no way to restrict \code{S} to be a 
\code{Symbol}. You could have a nonsense type like \code|mytype{ZZ, Poly}|. The code would not
operate, since none of the code using values of type \code{mytype} seriously expects \code{S} to be
a polynomial. It expects a symbol. 
...
for \code{div} over the floating point numbers. (Actually, ironically, Julia uses Euclid's original
definition of Euclidean division, even though that is not what happens in a Euclidean domain.)
...
We advise not ever using the \code{/} operator with core Julia types in Nemo, but always using 
\code{div}. This will not result in mathematical theorems being false. See the section above 
...
we define a function \code{PadicNumbers} for constructing it.
...
the prime $p$ for the $p$-adic type, so that there is effectively a different
type for each prime $p$. 

We name the symbol \code{S} carefully so that if the $p$-adic numbers are
constructed twice with the same prime $p$, the same type results. This is
especially important for the $p$-adics, which can be constructed without
actually constructing the field which they belong to (e.g. they can be
constructed using the $O(p^n)$ notation). 

In order to define operations on $p$-adic values with the same prime $p$, the
types of values constructed with the same $p$ must be the same.
...
A convenient way of constructing $p$-adic values, without first constructing
the type using the above function, is with the \code{O} constructor.
...
\desc{Construct the value $0 + O(p^n)$ given $m = p^n$. Note that $p$ and $n$
are determined automatically from the value $m$, which is assumed to be a prime
power. An exception results if $m$ is not found to be a prime power. Note that
at present we require $n > 0$. This will be extended in a future version of
Nemo.}
...
integer $a$. An exception will result if $a < 0$.}
...
d = 1 + 2*7 + 4*7^2 + O(7^3)
e = 13 + 357*ZZ(65537) + O(ZZ(65537)^12)
\end{lstlisting}

Beware that the expression \code{1 + 2*p + 3*p^2 + O(p^n)} is actually computed as
...
\code{1 + 7 + 2*7^2 + O(7^3)} is $3$. The precision is meaningless for an
exact value.}
...
\desc{Return $xy$ where $y$ is interpreted as an exact $p$-adic value. If
the result is unrepresentable, an exception results.}
...
\desc{Return $xy$ where $x$ is interpreted as an exact $p$-adic value. If
the result is unrepresentable, an exception results.}
