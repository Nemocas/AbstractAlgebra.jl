...
In contrast to parametric types, the prime example of a dependent type arises for a number 
...
type. Thus the type for $\Z/n\Z$ is a dependent type.
...
We might refer to this as static dependent typing versus dynamic dependent typing. Julia
has the former because it can be highly optimised. 

The usefulness of dependent types for Nemo is in performance. If the modulus $n$ can be
retained as part of the type and not part of the object, then it does not have to be carried
along at runtime. 
...
Finally, with specialised implementations of functions for particular rings Julia can 
directly call native C functions that implement the operations over those rings, with 
practically zero overhead.

The static dependent typing approach has some limitations. One cannot expect to generate
many dependent types programmatically, e.g. $\Z/p\Z$ for lots of values $p$, since the way
the optimiser works is to compile the same function once for each different type, i.e.
for each value of $p$ that is actually used in the program.

For this reason, one should not expect to implement multimodular algorithms, for example,
with $\Z/p\Z$ represented using a dependent type. Instead, one should implement such
algorithms over $\Z$ directly, interpreting the values as residues modulo the current
prime $p$, i.e. such algorithms have to be implemented the old-fashioned way.
...
Nemo comes equipped with a test suite. Each module defines a function which you can
call to run the tests for that module.

For example, for the \code{Rings} module, type
...
For the \code{Fields} module, type
...
the speed of your machine. However, they will run faster on a second run due to not
needing to be compiled by the Jit the second time.

If you encounter bugs in Nemo, please report them to the flint-devel Google group.
Note that Nemo currently requires the latest trunk version of flint from out GitHub
repository, or flint-2.5 (when available).
...
\desc{For \code{ZZ} this is just an alias for the \code{==} operator.}
