...
\desc{Given a non-singular $n\times n$ matrix over a ring and an $n\times 1$
column vector over the same ring, return a tuple $x, d$ consisting of an
$n\times 1$ column vector $x$ and a denominator $d$ such that $Ax = db$. The
...
\desc{Given a non-singular $n\times n$ matrix over a field and an $n\times 1$
column vector over the same field, return an $n\times 1$ column vector $x$
