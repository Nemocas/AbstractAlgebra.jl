...
Here is an example of irreducibility testing.
...
Here is an example of squarefree testing.
...
\begin{lstlisting}
valuation(z::nmod_poly, p::nmod_poly)
\end{lstlisting}

\desc{Return a tuple $(n, q)$ consisting of an integer $n$ and a polynomial
$q$ not divisible by $p$ such that $z = p^n \cdot q$. It is assumed that the
modulus is prime.}
...
\desc{Return the Howell normal form of $a$. The matrix $a$ must have at least as
...
\desc{Return the strong echeleon form of $a$. The matrix $a$ must have at least as
many rows as columns.}

\textbf{Examples.}

Here is an example of Howell and strong echelon form.

\begin{lstlisting}
   R = ResidueRing(ZZ, 12)
   S = MatrixSpace(R, 3, 3)

   a = S([8 5 5; 0 9 8; 0 0 10])

   b = howell_form(a)
   c = strong_echelon_form(a)
\end{lstlisting}
...
the first $n$ columns of $N$ are a $\Q$-basis for the nullspace of $a$.}

\textbf{Examples.}

Here is an example of computing the $\Q$-basis of a nullspace.
...
\desc{Return a tuple $(N, n)$ consisting of the nullity $n$ and a matrix $N$ with
$c$ rows and $r$ columns, where $c$ is the number of columns of $a$, such that 
the columns of $N$ are a $\Z$-basis for the nullspace of $a$.}
...
\begin{lstlisting}
hnf_modular_eldiv(a::fmpz_mat, m::fmpz)
\end{lstlisting}

\desc{Compute the Hermite normal form of $a$ given that $m$ is a multiple of
the largest elementary divisor of $a$. The matrix $a$ must have full rank.}
...
\begin{lstlisting}
   ...
   MM = hnf_modular_eldiv(A, fmpz(27))
   is_hnf(M) == true
\end{lstlisting}
...
\subsection{Flint matrices over $\Q$: \code{fmpq_mat}}

The Flint type for matrices over $\Q$ is \code{fmpq_mat}. Matrices
of this type are created using the \code{MatrixSpace} constructor in Nemo. 
...
Matrices in this matrix space have parent of type \code{FmpqMatSpace}.

We describe functions for \code{fmpq_mat} only where they differ from the functions
available for generic matrices.
...
\desc{Return the Gram-Schmidt orthogonalisation of $A$.}

\textbf{Examples.}

Here is an example of computing the Gram-Schmidt orthogonalisation.
...
\begin{lstlisting}
hilbert(S::FmpqMatSpace)
\end{lstlisting}

\desc{Return the Hilbert matrix for the given matrix space. This is the matrix with entries
$H_{i,j} = 1/(i + j - 1)$.}

\textbf{Examples.}

Here is an example of computing the Hilbert matrix.
...
\desc{Lift the given (truncated) $p$-adic value to the rationals.}

\begin{lstlisting}
lift(R::FlintIntegerRing, a::padic)
\end{lstlisting}

\desc{Lift the given (truncated) $p$-adic value to the integers.}
...
\subsection{Arb real polynomials ($\R[x]$): \code|arb\_poly|}

Note that some discrete properties of polynomials over
\code{arb} coefficients are upper or lower bounds.
For example, we define the degree of a polynomial with leading term
$[0 \pm 0.1] x^8$ to be 8, despite the fact that the leading coefficient
is not known to be nonzero.
Some operations (such as polynomial division)
will throw an error when the leading coefficient is not
certainly nonzero.

\begin{lstlisting}
R, x = PolynomialRing(RR::ArbField, s::AbstractString)
\end{lstlisting}

Throughout this section, we assume that \code{RR} is an \code{ArbField}
and that \code{R} is the corresponding polynomial ring
in the variable \code{x}.

\subsubsection{Constructors}

\begin{lstlisting}
R()
R(b::Union{Int,fmpz,fmpq,Float64,arb})
R(b::Array{arb, 1})
R(b::fmpz_poly)
R(b::fmpq_poly)
R(b::arb_poly)
...
\desc{Create a polynomial from a given constant value, array of coefficients,
or polynomial.}
...
\begin{lstlisting}
strongequal(x::arb_poly, y::arb_poly)
...
\begin{lstlisting}
shift_left(x::arb_poly, len::Int)

shift_right(x::arb_poly, len::Int)
\end{lstlisting}
...
divexact(x::arb_poly, y::Union{Int,fmpz,fmpq,Float64,arb})

//(x::arb_poly, y::Union{Int,fmpz,fmpq,Float64,arb})
\end{lstlisting}

\subsubsection{Polynomial division}

\begin{lstlisting}
divrem(x::arb_poly, y::arb_poly)

mod(x::arb_poly, y::arb_poly)

divexact(x::arb_poly, y::arb_poly)
\end{lstlisting}

\subsubsection{Truncation}

\begin{lstlisting}
truncate(a::arb_poly, n::Int)
\end{lstlisting}

\begin{lstlisting}
mullow(x::arb_poly, y::arb_poly, n::Int)
\end{lstlisting}

\subsubsection{Evaluation}

\begin{lstlisting}
evaluate(x::arb_poly, y::arb)

evaluate(x::arb_poly, y::acb)

evaluate(x::arb_poly, y::Union{Int,Float64,fmpq})

evaluate(x::arb_poly, y::fmpz)
\end{lstlisting}
...
evaluate2(x::arb_poly, y::Union{Int,Float64,fmpz,fmpq})
\end{lstlisting}

\subsubsection{Composition}

\begin{lstlisting}
compose(x::arb_poly, y::arb_poly)
\end{lstlisting}

\subsubsection{Derivative and integral}

\begin{lstlisting}
derivative(x::arb_poly)

integral(x::arb_poly)
\end{lstlisting}

\subsubsection{Multipoint evaluation and interpolation}
...
\begin{lstlisting}
evaluate_iter(x::arb_poly, b::Array{arb, 1})

evaluate_fast(x::arb_poly, b::Array{arb, 1})

evaluate(x::arb_poly, b::Array{arb, 1})
\end{lstlisting}

\begin{lstlisting}
interpolate_newton(R::ArbPolyRing, xs::Array{arb, 1}, ys::Array{arb, 1})

interpolate_barycentric(R::ArbPolyRing, xs::Array{arb, 1}, ys::Array{arb, 1})

interpolate_fast(R::ArbPolyRing, xs::Array{arb, 1}, ys::Array{arb, 1})

interpolate(R::ArbPolyRing, xs::Array{arb, 1}, ys::Array{arb, 1})
\end{lstlisting}

\subsection{Arb complex polynomials ($\C[x]$): \code|acb\_poly|}

\subsubsection{Constructors}

\begin{lstlisting}
R()
R(b::Union{Int,fmpz,fmpq,Float64,Complex{Float64},Complex{Int},arb,acb})
R(b::Array{acb, 1})
R(b::fmpz_poly)
R(b::fmpq_poly)
R(b::arb_poly)
...
\begin{lstlisting}
strongequal(x::acb_poly, y::acb_poly)
...
\begin{lstlisting}
shift_left(x::acb_poly, len::Int)

shift_right(x::acb_poly, len::Int)
\end{lstlisting}
...
divexact(x::acb_poly, y::Union{Int,fmpz,fmpq,Float64,arb,acb})

//(x::acb_poly, y::Union{Int,fmpz,fmpq,Float64,arb,acb})
\end{lstlisting}

\subsubsection{Polynomial division}

\begin{lstlisting}
divrem(x::acb_poly, y::acb_poly)

mod(x::acb_poly, y::acb_poly)

divexact(x::acb_poly, y::acb_poly)
\end{lstlisting}

\subsubsection{Truncation}

\begin{lstlisting}
truncate(a::acb_poly, n::Int)
\end{lstlisting}

\begin{lstlisting}
mullow(x::acb_poly, y::acb_poly, n::Int)
\end{lstlisting}

\subsubsection{Evaluation}

\begin{lstlisting}
evaluate(x::acb_poly, y::acb)

evaluate(x::acb_poly, y::Union{Int,Float64,fmpq,arb})

evaluate(x::acb_poly, y::fmpz)
\end{lstlisting}

\begin{lstlisting}
evaluate2(x::acb_poly, y::acb)

evaluate2(x::acb_poly, y::Union{Int,Float64,fmpz,fmpq,arb})
\end{lstlisting}

\subsubsection{Composition}

\begin{lstlisting}
compose(x::acb_poly, y::acb_poly)
\end{lstlisting}

\subsubsection{Derivative and integral}

\begin{lstlisting}
derivative(x::acb_poly)
...
\begin{lstlisting}
evaluate_iter(x::acb_poly, b::Array{acb, 1})

evaluate_fast(x::acb_poly, b::Array{acb, 1})

evaluate(x::acb_poly, b::Array{acb, 1})
\end{lstlisting}

\begin{lstlisting}
interpolate_newton(R::AcbPolyRing, xs::Array{acb, 1}, ys::Array{acb, 1})

interpolate_barycentric(R::AcbPolyRing, xs::Array{acb, 1}, ys::Array{acb, 1})

interpolate_fast(R::AcbPolyRing, xs::Array{acb, 1}, ys::Array{acb, 1})

interpolate(R::AcbPolyRing, xs::Array{acb, 1}, ys::Array{acb, 1})
\end{lstlisting}
