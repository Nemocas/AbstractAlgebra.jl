...
is a IPython derivative, even on the web, via the IPython notebook!
...
a in a multitude of heap allocations. It also saves on object creation, as only one heap allocated
...
be used. And things like array indices will usually require an annotation or some sort (expensive)
...
The \code{div} operator provides integer floor division.

The carrot operator is for powering, not xor as in C. This is great for mathematicians, especially
when creating polynomials.
...
Arrays can too, in which case the types will be made the generic type \code{Any}. 
