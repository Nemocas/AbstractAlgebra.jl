...
type Poly{T <: Ring, S} <: Ring
   data :: Union(fmpz_poly, fmpz_mod_poly, 
                            fmpq_poly, fq_poly, PolyStruct{T})
end
\end{lstlisting}
...
\desc{Return the degree one, monic monomial generator of the power series ring.}
...
*{T <: Ring, S}(a::T, b::PowerSeries{T, S})
\end{lstlisting}
...
=={T <: Ring, S}(x::PowerSeries{T, S}, y::T)
\end{lstlisting}
...
=={T<: Ring, S}(x::T, y::PowerSeries{T, S})
\end{lstlisting}
...
The finite field type in Nemo is \code{FField}. The uninterested user can skip
description of the internals of this type, as we provide functions for 
constructing the type.

As with the \code{ZZ} type, the \code{FField} type in Nemo simply represents a
...
Here is the Julia definition of the \code{FField} type.
...
The \code{FField} type is parameterised by a symbol \code{S} which holds on to
a flint context object with information such as the polynomial used to
...
\code{fq_ctx} type which stores this context object.

The \code{FField} type belongs to the \code{Field} typeclass, and therefore
elements of a finite field can be passed to any function which operates on
field or ring elements.
...
\begin{lstlisting}
characteristic{S}(::Type{FField{S}})
\end{lstlisting}
...
\begin{lstlisting}
divexact{S}(x::FField{S}, y::FField{S})
/{S}(x::FField{S}, y::FField{S})
\end{lstlisting}
