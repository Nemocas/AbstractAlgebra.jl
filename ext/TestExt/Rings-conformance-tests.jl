# very generic testing: just define ConformanceTests.pseudo_random_element(R) to produce elements of R,
...
function ConformanceTests.test_NCRing_interface(R::AbstractAlgebra.NCRing; reps = 50)
   ...
   @testset "NCRing interface for $(R) of type $(typeof(R))" begin
      ...
      @testset "Basic functions" begin
         ...
         for i in 1:reps
            a = pseudo_random_element(R)::T
            b = pseudo_random_element(R)::T
            c = pseudo_random_element(R)::T
            A = deepcopy(a)
            B = deepcopy(b)
            C = deepcopy(c)
            @test equality(+(a), a)
      ...
      if is_exact_type(T)
        @testset "Adhoc operations with $S" for S in adhoc_partner_rings(R)
          ...
          r1 = one(R)
          for i in 1:reps
            s2 = pseudo_random_element(S)
            r2 = R(s2)
            x = pseudo_random_element(R)
...
function ConformanceTests.test_EuclideanRing_interface(R::AbstractAlgebra.Ring; reps = 20)
   ...
   @testset "Euclidean Ring interface for $(R) of type $(typeof(R))" begin
      ...
      for i in 1:reps
         f = pseudo_random_element(R)::T
         g = pseudo_random_element(R)::T
         m = pseudo_random_element(R)::T
         if iszero(m)
            m = one(R)
         end
   ...
   @testset "Poly interface for $(Rx) of type $(typeof(Rx))" begin
      ...
      if R isa AbstractAlgebra.Field
         ...
         @testset "Half-GCD" begin
            for i in 1:reps
               a = pseudo_random_element(Rx)
               b = pseudo_random_element(Rx)
               for j in 1:8
                  q = pseudo_random_element(Rx)
                  a, b = q*a + b, a
               end
               g, s, t = gcdx(a, b)
               @test g == gcd(a, b)
