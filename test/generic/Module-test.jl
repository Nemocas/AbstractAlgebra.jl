function test_module_intersection()
   print("Generic.Module.intersection...")

   R = ZZ

   for iter = 1:100
      rk = rand(1:5)

      M = FreeModule(R, rk)

      ngens1 = rand(1:rk)
      gens1 = [M([R(rand(-10:10)) for i in 1:rk]) for j in 1:ngens1]
      M1, f1 = Submodule(M, gens1)

      ngens2 = rand(1:rk)
      gens2 = [M([R(rand(-10:10)) for i in 1:rk]) for j in 1:ngens2]
      M2, f2 = Submodule(M, gens2)

      ngens3 = rand(1:rk)
      gens3 = [M([R(rand(-10:10)) for i in 1:rk]) for j in 1:ngens3]
      M3, f3 = Submodule(M, gens3)
      
      I1, g1 = intersect(M1, M2)
      I2, g2 = intersect(I1, M3)

      J1, h1 = intersect(M2, M3)
      J2, h2 = intersect(M1, J1)

      @test I2 == J2
   end
   
   println("PASS")
end

function test_module()
   test_module_intersection()

   println("")
end
